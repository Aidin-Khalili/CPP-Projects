...
public:
	...
	bool Is_eaten_food(Food*food)
	{
		if (snake->Get_head_x() == food->Get_x() && snake->Get_head_y() == food->Get_y())
			return true;
		else
			return false;
	}
	...
	void start()
	{
		...
		while (true)
		{
			...
			if (!Is_eating_food())
				snake->Move();
			if (snake->Clash_body() || Clash_to_wall() || score < 0)
			{
				system("cls");
				Gotoxy(12, 6);
